...
import { getTokens, refreshTokens, removeTokens, clearAuthData } from '@/lib/auth/tokens'
...
interface ErrorHandler {
    ...
    showAuthError: (message: string, action?: { label: string; onPress: () => void }) => void;
}
...
const isPublicEndpoint = (url: string): boolean => {
    ...
    if (url.match(/^\/videos\/[^\/]+$/) && !url.includes('/like') && !url.includes('/comment') && !url.includes('/share')) {
        return true;
    }
...
uploadClient.interceptors.request.use(authInterceptor, (error) => Promise.reject(error));
    ...
    if (status === 401) {
        ...
        if (
            ...
            try {
                ...
                return isUpload ? uploadClient(originalRequest) : apiClient(originalRequest);
            } catch (refreshError) {
...
uploadClient.interceptors.response.use((response) => response, responseInterceptor);
