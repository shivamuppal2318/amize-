...
export const usePostApi = () => {
  ...
  const createPost = useCallback(
    async (data: CreatePostData): Promise<PostResponse> => {
      try {
        setLoading(true);
   ...
   */
  const createMultiplePosts = useCallback(
    async (
      uploadIds: string[],
  ...
  const createSlideshow = useCallback(
    async (data: CreateSlideshowData): Promise<PostResponse> => {
      try {
        setLoading(true);
  ...
  const getVideos = useCallback(
    async (params?: {
      ...
      try {
        const response = await apiClient.get(VIDEO_ENDPOINTS.VIDEOS, {
          params,
        });
        return response.data;
      } catch (error: any) {
  ...
  const shareVideo = useCallback(async (videoId: string, platform: string) => {
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "Failed to share video";
      throw new Error(errorMessage);
    }
  }, []);
