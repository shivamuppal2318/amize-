...
import { uploadService, FileUploadOptions, UploadResponse } from '@/lib/api/uploadService';
...
export const useUploadApi = () => {
    ...
    const uploadMultipleFiles = useCallback(async (
        ...
        const updateOverallProgress = () => {
            const totalProgress = Object.values(fileProgresses).reduce((sum, progress) => sum + progress, 0);
        ...
        if (errors.length > 0) {
            if (errors.length === files.length) {
                ...
                toast.show(
                    ...
                    `${results.filter(r => r).length} of ${files.length} files uploaded successfully`
                );
            }
        }
     ...
     */
    const getUploads = useCallback(async (params?: {
        uploadType?: string;
        status?: string;
        page?: number;
        limit?: number;
    ...
    const getUploadById = useCallback(async (id: string) => {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to fetch upload details';
            toast.show('Error', errorMessage);
            throw error;
        }
    }, [toast]);
