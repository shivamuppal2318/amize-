...
import { User, RegisterRequest, VerifyCodeResponse, ResendCodeResponse } from '@/lib/api/types';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const initializeSocketForUser = async (userData: User, onboardingComplete: boolean) => {
        try {
            ...
            if (userData.verified && onboardingComplete && !isInSignupFlow) {
                ...
                console.log('[AuthContext] Skipping socket initialization - user not ready');
            }
        } catch (error) {
    ...
    const validateAuthState = useCallback(async (): Promise<boolean> => {
        try {
            ...
            if (!isValid && (hasValidTokens || userJSON)) {
                ...
                console.log('[AuthContext] Inconsistent auth state detected, clearing data');
                await clearAuthData();
                return false;
            }
    ...
    const syncAuthState = useCallback(async (forceRefresh = false) => {
        try {
            const isValid = await validateAuthState();

            if (!isValid && isAuthenticated && !forceRefresh) {
                // Try to refresh tokens once
                console.log('[AuthContext] Auth state invalid, attempting token refresh');
    ...
    useEffect(() => {
        const initializeAuth = async () => {
            try {
                ...
                const onboardingCompleted = await secureStorage.get(STORAGE_KEYS.ONBOARDING_COMPLETED);
    ...
    const updateUser = useCallback((data: Partial<User>) => {
        ...
        if (user) {
            ...
            secureStorage.set(STORAGE_KEYS.USER_DATA, JSON.stringify(updatedUser))
                .then(() => console.log('[AuthContext] User data saved'))
                .catch(error => console.error('[AuthContext] Error saving user data:', error));
    ...
    const login = useCallback(async (email: string, password: string): Promise<LoginResult> => {
        ...
        try {
            ...
            if (response.success && response.token && response.refreshToken && response.user) {
                ...
                // Store user data
                await secureStorage.set(STORAGE_KEYS.USER_DATA, JSON.stringify(response.user));
    ...
    // Register
    const register = useCallback(async (userData: RegisterRequest): Promise<RegisterResult> => {
        setLoading(true);
        ...
        try {
            ...
            if (response.success) {
                console.log(`[AuthContext] Registration initiated for ${userData.email}`);
    ...
    const verifyCode = useCallback(async (email: string, code: string): Promise<VerifyCodeResponse> => {
        setLoading(true);
    ...
    const resendVerificationCode = useCallback(async (email: string): Promise<ResendCodeResponse> => {
        ...
        try {
            ...
            console.error('[AuthContext] Error resending verification code:', error);
            return {
                success: false,
