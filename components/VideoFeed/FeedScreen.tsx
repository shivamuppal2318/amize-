...
const AnimatedFlatList = Animated.createAnimatedComponent(FlatList<VideoItemData>);
        ...
        <SafeAreaView style={loaderStyles.mainContainer}>
            <View style={loaderStyles.container}>
                ...
                <View style={loaderStyles.textContainer}>
                    ...
                    <Text style={loaderStyles.subtitleText}>Discovering amazing content for you...</Text>
                </View>
            </View>
        </SafeAreaView>
    );
    ...
    return (
        ...
        >
            <View style={emptyStyles.iconContainer}>
                <View style={emptyStyles.iconBackground}>
                    {renderIcon()}
                </View>
            </View>
...
const FeedScreen: React.FC = () => {
    ...
    const log = useRef(createDebouncedFunction((message: string, ...args: any[]) => {
    ...
    const handleEndReached = useCallback(() => {
        ...
        if (now - state.lastEndReachedTime < 2000) {
            ...
            if (state.consecutiveEndReached > 2) {
                log(`🚫 onEndReached blocked: too many consecutive calls (${state.consecutiveEndReached})`);
                return;
            }
        } else {
        ...
        // Basic checks
        if (!canLoadMore) {
            log(`⏸️ onEndReached skipped: canLoadMore=${canLoadMore}, hasMore=${hasMore}, loading=${loading}`);
            return;
        }
        ...
        if (now < loadMoreState.throttleUntil) {
            log(`⏸️ onEndReached skipped: throttled until ${new Date(loadMoreState.throttleUntil).toLocaleTimeString()}`);
            return;
        }
    ...
    useEffect(() => {
        if (!params.current.videoId || videos.length === 0 || navigationHandled.current) {
            return;
        }
        ...
        const handleVideoNavigation = async () => {
            ...
            const videoIndex = videos.findIndex(v => v.id === params.current.videoId);
            ...
            if (videoIndex !== -1) {
                ...
                try {
                    const specificVideo = await loadSpecificVideo(params.current.videoId!);
                    if (specificVideo) {
    ...
    useEffect(() => {
        ...
        const prevVideo = lastFocusedIndex.current >= 0 ? videos[lastFocusedIndex.current] : null;
    ...
    const handleMomentumScrollEnd = useCallback((event: any) => {
        ...
        if (newIndex !== focusedIndex && newIndex >= 0 && newIndex < videos.length) {
    ...
    const handleLike = useCallback(async (videoId: string, liked: boolean) => {
    ...
    const handleShare = useCallback(async (videoId: string, platform?: string) => {
        ...
        if (platform) {
            try {
                ...
                await shareVideo(videoId, (platform as any) || 'copy_link', (count) => {
    ...
    const handleSearchedVideoPress = useCallback(async (videoId: string) => {
        setSearchModalVisible(false);
    ...
    const renderItem = useCallback(({item, index}: { item: VideoItemData; index: number }) => {
            ...
            <View style={styles.videoItemContainer}>
                {shouldLoad ? (
                    <VideoItem
                        ...
                        onBookmark={() => {}}
                        onProfilePress={() => router.push(`/(tabs)/profile/${item.user.id}`)}
                        onAuthRequired={() => {}}
                        index={index}
                    />
                ) : (
            ...
            <View style={styles.loadingFooter}>
                ...
                {!loading && hasMore && (
                    ...
                    >
                        ...
                        <Text style={styles.debugText}>
                            NearEnd: {isNearEnd.toString()} | Focused: {focusedIndex}/{videos.length}
                        </Text>
                        <Text style={styles.debugText}>
                            InProgress: {loadMoreState.inProgress.toString()} | Retries: {loadMoreState.retryCount}
                        </Text>
                        <Text style={styles.debugText}>
                            Requests: {perfStats.successfulRequests}/{perfStats.totalRequests} | Avg: {Math.round(perfStats.averageResponseTime)}ms
                        </Text>
                    </TouchableOpacity>
                )}
    ...
    }, [loading, hasMore, canLoadMore, isNearEnd, focusedIndex, videos.length, getLoadMoreState, getPerformanceStats, handleManualLoadMore]);
        ...
        <View style={styles.tabContainer}>
            ...
            >
                <View style={styles.tabIconContainer}>
                    ...
                    {!isAuthenticated && <Lock size={12} color="rgba(255,255,255,0.5)" style={{marginLeft: 4}}/>}
                </View>
                {feedType === 'following' && (
    ...
    const renderEmpty = useCallback(() => {
        ...
        if (videos.length === 0) {
            ...
            if (feedType === 'following') {
                if (isAuthenticated) {
                    message = "You're not following any creators yet. Discover amazing creators in the For You feed!";
                    showAction = true;
                    actionText = "Explore For You";
                    actionHandler = () => handleTabPress('forYou');
                } else {
                    message = "Follow your favorite creators and never miss their latest content";
                    icon = "login";
                    showLoginAction = true;
                }
            } else if (feedType === 'subscribed') {
                if (isAuthenticated) {
                    message = "No premium content available. Subscribe to creators to unlock exclusive videos!";
                    showAction = true;
                    actionText = "Browse Creators";
                    actionHandler = () => handleTabPress('forYou');
                } else {
                    message = "Access exclusive premium content from your favorite creators";
                    icon = "login";
                    showLoginAction = true;
                }
            } else if (feedType === 'forYou') {
                message = "We're preparing personalized content for you. Check back in a moment!";
                showAction = true;
                actionText = "Refresh";
        ...
        return null;
    }, [loading, error, videos.length, refresh, feedType, isAuthenticated, handleTabPress]);
    ...
    return (
        <View style={styles.container}>
            ...
            <AnimatedFlatList
                ...
                onEndReachedThreshold={0.2} // Increased threshold
