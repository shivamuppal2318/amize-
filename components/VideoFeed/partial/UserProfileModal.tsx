...
export const UserProfileModal = ({
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      const creatorStatusResponse = await CreatorAPI.getUserCreatorStatus(
        userId
      );
      ...
      if (creatorStatusResponse.success) {
        ...
        if (userIsCreator && userIsEligible) {
          ...
          if (creatorPlansResponse.success) {
            setCreator({
              ...
              category:
                creatorPlansResponse.creator.category || "Content Creator",
              subscriberCount: creatorPlansResponse.creator.subscriberCount,
            });

            // Set the plans from the response
            if (
              creatorPlansResponse.plans &&
              Array.isArray(creatorPlansResponse.plans)
            ) {
            ...
            if (
              !initialAvatar &&
              creatorPlansResponse.creator.profilePhotoUrl
            ) {
            ...
            if (isAuthenticated && user) {
              try {
                const isUserSubscribed =
                  await CreatorAPI.checkSubscriptionStatus(user.id, userId);
          ...
          creatorData.subscriptionPlans &&
          Array.isArray(creatorData.subscriptionPlans)
        ) {
        ...
        console.error(
          "Creator status API returned error:",
  ...
  if (loading) {
    return (
      <CustomModal visible={visible} onClose={onClose} title="Loading Profile">
        ...
        >
          ...
          >
            ...
            <Text style={styles.thankYouText}>
              You are now subscribed to {creator?.username || username}'s
              content.
            </Text>
          </LinearGradient>
        </Animated.View>
                      ...
                      {selectedPlan === plan.id && (
                        ...
                        >
                          ...
                          <Text style={styles.paymentInfoText}>
                            You'll be charged{" "}
                            {plan.currency === "USD" ? "$" : plan.currency}
                            {plan.price.toFixed(2)} {plan.intervalType}.
                          </Text>
                        </Animated.View>
                      )}
                    </TouchableOpacity>
            ...
            <View style={styles.profileHeader}>
              <View style={styles.avatarSection}>
                <View style={styles.avatarContainer}>
                  <Image
                    source={{
                      uri: avatar || 'https://cdn-icons-png.flaticon.com/512/219/219983.png',
                    }}
