...
const VideoItem: React.FC<VideoItemProps> = ({
    ...
    const log = useRef((message: string, ...args: any[]) => {
        console.log(`🎥 [${componentId.current}] [Index: ${index}] [Focused: ${focused}] ${message}`, ...args);
    }).current;
    ...
    const videoSource = useMemo((): VideoSource => {
        ...
        if (localPath && isPreloaded) {
            console.log(`🎥 [${componentId.current}] Using cached video: ${localPath}`);
            return { uri: localPath };
        } else {
            console.log(`🎥 [${componentId.current}] Using remote video: ${item.video.uri}`);
    ...
    useEffect(() => {
        ...
        if (statusChangeInProgress.current || (now - lastStatusChange.current < 200)) {
            return;
        }
            ...
            switch (status) {
                ...
                case 'readyToPlay':
                    ...
                    if (focused && !forcePause && !manuallyPaused.current) {
                        ...
                        setTimeout(() => {
                            if (isMounted.current && focused && !forcePause && !manuallyPaused.current && playerReady.current) {
                                player.play();
    ...
    useEffect(() => {
        ...
        log(`🎮 Focus/Pause logic - shouldPause: ${shouldPause}, focused: ${focused}, forcePause: ${forcePause}, manuallyPaused: ${manuallyPaused.current}, isPlaying: ${player.playing}`);

        if (shouldPause && player.playing) {
            player.pause();
            log(`⏸️ Pausing video - forcePause: ${forcePause}, focused: ${focused}`);
        } else if (!shouldPause && focused && !manuallyPaused.current && !player.playing) {
    ...
    const handleVideoPress = useCallback(() => {
        ...
        log(`👆 Video pressed - current playing state: ${player.playing}, manuallyPaused: ${manuallyPaused.current}`);
    ...
    const handleViewFullProfile = useCallback((userId: string) => {
    ...
    const handleSocialSharePress = useCallback((platform: ApiSharePlatform) => {
        setShareModalVisible(false);
        onShare(platform);
    ...
    const LikeIcon = ({ color = '#FF5A5F' }: { color?: string }) => (
        <Svg width="24" height="24" viewBox="0 0 31 26" fill="none">
            <Path d="M0.870605 9.32497C0.870605 16.4167 6.73227 20.1958 11.0231 23.5784C12.5373 24.7721 13.9956 25.8959 15.4539 25.8959C16.9123 25.8959 18.3706 24.7721 19.8848 23.5784C24.1756 20.1958 30.0373 16.4167 30.0373 9.32497C30.0373 2.23318 22.0161 -2.79617 15.4539 4.02178C8.89167 -2.79617 0.870605 2.23318 0.870605 9.32497Z" fill={color} />
        </Svg>
    );

    const CommentIcon = ({ color = '#FFF' }: { color?: string }) => (
        <Svg width="24" height="24" viewBox="0 0 31 30" fill="none">
            <Path d="M15.4539 29.4565C23.508 29.4565 30.0373 22.9272 30.0373 14.8731C30.0373 6.81897 23.508 0.289795 15.4539 0.289795C7.39978 0.289795 0.870605 6.81897 0.870605 14.8731C0.870605 17.206 1.41838 19.4109 2.39232 21.3664C2.65114 21.886 2.73729 22.4799 2.58724 23.0407L1.71864 26.2871C1.34157 27.6963 2.63083 28.9854 4.04007 28.6084L7.28638 27.7399C7.84717 27.5898 8.44112 27.676 8.96076 27.9347C10.9161 28.9087 13.121 29.4565 15.4539 29.4565Z" fill={color} />
        </Svg>
    );

    const ShareIcon = ({ color = '#FFF' }: { color?: string }) => (
        <Svg width="24" height="24" viewBox="0 0 31 30" fill="none">
            <Path d="M29.8617 4.64445L21.5784 27.8611C21.3803 28.4377 21.0064 28.9377 20.5094 29.2908C20.0124 29.644 19.4172 29.8325 18.8076 29.8299C18.2049 29.8367 17.615 29.6567 17.1188 29.3146C16.6226 28.9725 16.2446 28.4851 16.0367 27.9194L12.9451 19.8694L15.1909 17.6236L18.8221 13.9924C19.0968 13.7178 19.251 13.3453 19.251 12.957C19.251 12.5686 19.0968 12.1961 18.8221 11.9215C18.5475 11.6469 18.1751 11.4927 17.7867 11.4927C17.3984 11.4927 17.0259 11.6469 16.7513 11.9215L13.1201 15.5528L10.8159 17.8132L2.78048 14.707C2.18042 14.5262 1.65454 14.157 1.28074 13.654C0.906931 13.151 0.705078 12.5409 0.705078 11.9142C0.705078 11.2876 0.906931 10.6775 1.28074 10.1745C1.65454 9.67152 2.18042 9.30229 2.78048 9.12154L26.0555 0.838203C26.5877 0.619525 27.1728 0.563177 27.737 0.676251C28.3012 0.789324 28.8194 1.06677 29.2263 1.47366C29.6332 1.88055 29.9106 2.39871 30.0237 2.96292C30.1368 3.52714 30.0804 4.11219 29.8617 4.64445Z" fill={color} />
        </Svg>
    );
