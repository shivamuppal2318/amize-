...
const MessagingSection: React.FC<MessagingSectionProps> = ({ autoOpenConversation }) => {
    // State
    ...
    useEffect(() => {
        ...
        if (autoOpenConversation &&
            ...
            let conversation = conversations.find(c => c.id === autoOpenConversation.id);
    ...
    const handleTypingStart = () => {
        if (!selectedConversation?.participants || !user) {
            console.warn('[MessagingSection] Cannot start typing - missing conversation participants or user');
            return;
        }
        ...
        if (otherParticipant) {
            ...
            console.warn('[MessagingSection] No other participant found for typing', {
                conversationId: selectedConversation.id,
                currentUserId: user.id,
                participants: selectedConversation.participants.map(p => ({ id: p.id, username: p.username }))
            });
        }
    };
    ...
    const handleTypingStop = () => {
        if (!selectedConversation?.participants || !user) {
            console.warn('[MessagingSection] Cannot stop typing - missing conversation participants or user');
            return;
        }
