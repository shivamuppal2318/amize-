...
const ConversationItem: React.FC<ConversationItemProps> = ({
    ...
    // Format timestamp to display relative time with better error handling
    ...
    const isOnline = item.isOnline || (item.participants && item.participants.some(p => p.isOnline));
                        ...
                        <View style={styles.avatarSection}>
                            <View style={styles.avatarContainer}>
                                <Image
                                    ...
                                    onError={() => {
                                        console.warn('Failed to load avatar for conversation:', item.id);
                                    }}
                                />
                                {isOnline && (
                        ...
                        <View style={styles.conversationDetails}>
                            <View style={styles.conversationHeader}>
                                <Text
                                    ...
                                    numberOfLines={1}
                                >
                                    {searchText ? highlightSearchText(item.name, searchText) : item.name}
                                </Text>
                            ...
                            <View style={styles.messageRow}>
                                ...
                                >
                                    {searchText ?
                                        highlightSearchText(truncateMessage(item.lastMessage), searchText) :
                            ...
                            {item.type === 'group' && item.participants && (
                                <Text style={styles.participantsCount}>
                                    ...
                                    {item.participants.filter(p => p.isOnline).length > 0 &&
                                        ` • ${item.participants.filter(p => p.isOnline).length} online`
                                    }
                                </Text>
                            )}
                        </View>
