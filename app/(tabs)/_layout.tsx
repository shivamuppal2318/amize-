...
export default function TabLayout() {
    ...
    const TabBarButton = ({label, icon, onPress, focused}: TabBarButtonProps) => {
        if (label === 'Create') {
            return (
                <TouchableOpacity
        ...
        return (
            ...
            >
                ...
                <Text
                    style={[
                        styles.tabLabel,
                        focused ? {color: '#FF4D67'} : {color: '#888'}
                    ]}
                >
                    {label}
                </Text>
    ...
    // Custom HomeIcon component using react-native-svg components
    ...
    // Updated ContentIcon to toggle between + and X
    ...
    return (
        <>
            <Tabs
                screenOptions={{
                    ...
                    tabBarStyle: {
                        ...
                        borderTopWidth: 0,
                        elevation: 0,
                        height: 40 + insets.bottom, // Add safe area bottom insets
                        paddingTop: 5,
                    },
                ...
                tabBar={({navigation, state, descriptors}) => (
                    <View style={[
                        ...
                        {state.routes
                            .filter(route => ['index', 'explore', 'create', 'inbox', 'profile'].includes(route.name))
                            .map((route, index) => {
                                ...
                                // Enhanced profile button handling with auth prompt
                                if (route.name === 'profile') {
                                    onPress = () => {
                                        if (isAuthenticated && user) {
                                            // Navigate directly to current user's profile
                                            navigation.navigate('profile', {
                                                screen: '[id]',
                                                params: { id: user.id }
                                            });
                                        } else {
                                            // Show auth prompt instead of direct navigation
                                            showAuthModal('access_profile');
                                        }
                                    };
                                }

                                // Enhanced inbox button handling with auth prompt
                                if (route.name === 'inbox') {
                                    onPress = () => {
                                        if (isAuthenticated) {
                                            // Navigate to inbox for authenticated users
                                            const event = navigation.emit({
                                                type: 'tabPress',
                                                target: route.key,
                                                canPreventDefault: true,
                                ...
                                let icon;
                                if (route.name === 'index') {
                                    icon = <HomeIcon color={isFocused ? '#FF4D67' : '#888'}/>;
                                } else if (route.name === 'explore') {
                                    icon = <DiscoverIcon color={isFocused ? '#FF4D67' : '#888'}/>;
                                } else if (route.name === 'create') {
                                    icon = <ContentIcon color='#FF4D67'/>;
                                } else if (route.name === 'inbox') {
                                    // Visual indication for auth-required tabs
                                    const iconColor = !isAuthenticated ?
                                        (isFocused ? '#FF4D67' : 'rgba(136, 136, 136, 0.6)') :
                                        (isFocused ? '#FF4D67' : '#888');
            ...
            {/* Create Modal - Only show if authenticated */}
            {isAuthenticated && (
                <CreateOptionModal
