...
import { ProfileUpdateData } from '@/lib/api/profileApi';
...
export default function EditProfileScreen() {
    ...
    // Use the profile editor hook for all state management
    const {
        profile,
        formData,
        loading,
    ...
    // Load profile data on mount only if needed
    useEffect(() => {
        if (!profile) {
            loadProfile();
        }
    ...
    const handleImagePickerLaunch = async (source: 'library' | 'camera') => {
        ...
        try {
            ...
            if (permission.status !== 'granted') {
                // On iOS, if permissions are denied, guide the user to settings.
            ...
            // 3. Launch the picker only if permissions are granted
            ...
            if (source === 'camera') {
                ...
                result = await ImagePicker.launchImageLibraryAsync({
                    mediaTypes: 'images', // Corrected value
                    allowsEditing: true,
                    aspect: [1, 1],
                    quality: 0.8,
                });
            ...
            Alert.alert('Error', 'An unexpected error occurred while trying to select an image. Please try again.');
        }
    };
        ...
        return (
            <SafeAreaView className="flex-1 bg-[#1a1a2e]">
                <View className="flex-1 justify-center items-center px-6">
                    <Text className="text-white text-xl font-bold mb-4">Profile Not Found</Text>
                    <Text className="text-gray-400 text-center mb-6">Unable to load your profile. Please try again.</Text>
                    <TouchableOpacity
                        onPress={loadProfile}
                ...
                <View className="items-center px-6 pb-6">
                    <View className="relative">
                        ...
                        <TouchableOpacity
                            ...
                            disabled={uploading}
                            className="absolute bottom-0 right-0 bg-[#FF5A5F] p-3 rounded-full border-4 border-[#1a1a2e]"
                        >
                    ...
                    <TouchableOpacity
                        onPress={handlePickImage}
                        disabled={uploading}
                        className="bg-[#FF5A5F]/20 border border-[#FF5A5F]/50 px-6 py-3 rounded-full"
                    >
                        <Text className="text-[#FF5A5F] font-semibold">
                ...
                <View className="px-6 mb-8">
                    ...
                    {socialFields.map((field, index) => (
                        <EditProfileItem
                            ...
                            icon={field.icon}
                            label={field.label}
                            value={field.value ? `@${field.value}` : `Add ${field.label} handle`}
                            onPress={() => handleFieldEdit(field)}
                            hasValue={!!field.value}
